...
export class FakerData {
    ...
    static getOpportunityName(): string {
        return `AutoOpp ${faker.string.alphanumeric(6)}`
    }

    static getCloseDate(): string {
        return faker.date.soon({ days: 30 }).toLocaleDateString('en-GB'); // DD/MM/YYYY
    }
    
    static getAmount(): string {
        return faker.finance.amount(1000, 10000, 0)
    }
}

export function getOpportunityName(): string {
    return `AutoOpp ${faker.string.alphanumeric(6)}`
}

export function getCloseDate(): string {
    return faker.date.soon({ days: 30 }).toLocaleDateString('en-GB'); // DD/MM/YYYY
}
