...
export abstract class PlaywrightWrapper {
  ...
  */
    ...
    */
    ...
    async clickLast(locator: string, name: string, type: string) {
        await test.step(`The ${name} ${type} clicked`, async () => {
            await this.page.waitForSelector(locator, { state: 'visible' });
    ...
    async clickFirst(locator: string, name: string, type: string) {
        await test.step(`The ${name} ${type} clicked`, async () => {
            await this.page.waitForSelector(locator, { state: 'visible' });
    ...
    async storeState(path: string): Promise<void> {
        await test.step(`Storing browser state to: ${path}`, async () => {
            try {
    ...
    */
    async getInnerText(locator: string): Promise<string> {
        return await test.step(`Getting inner text from: ${locator}`, async () => {
    ...
    */
    async getTextContent(locator: string): Promise<string | null | any> {
        return await test.step(`Getting text content from: ${locator}`, async () => {
    ...
    */
    async getText(locator: string): Promise<string> {
        return await test.step(`Getting input value from: ${locator}`, async () => {
    ...
    * @returns {Promise<string | null | undefined>} - The value of the attribute, or null if the attribute does not exist.
    */
    async fetchattribute(locator: string, attName: string) {
        return await test.step(`Fetching attribute '${attName}' from: ${locator}`, async () => {
    ...
    */
    async multipleWindowsCount(): Promise<number> {
        return await test.step(`Getting open browser windows count`, async () => {
    ...
    * 
    ...
    async focusWindow(locator: string): Promise<any> {
        return await test.step(`Focusing on new window opened by: ${locator}`, async () => {
    ...
    async switchToWindow(windowTitle: any, locator: string): Promise<Page | null> {
        return await test.step(`Switching to window with title: ${windowTitle}`, async () => {
    ...
    async acceptAlert(Data: string) {
        await test.step(`Accepting alert dialog with data: ${Data}`, async () => {
    ...
    async doubleClick(locator: string, name: string) {
        await test.step(`The ${name} double clicked`, async () => {
            await this.page.locator(locator).click({ force: true })
            await this.page.locator(locator).click({ force: true })
        })
    }
    ...
    async verification(locator: string, expectedTextSubstring: string) {
        await test.step(`Verifying element contains text: ${expectedTextSubstring}`, async () => {
    ...
    async waitForElementHidden(locator: string, type: string) {
        await test.step(`Waiting for element to be hidden: ${type}`, async () => {
            try {
                await this.wait('minWait')
    ...
    async validateElementVisibility(locator: any, elementName: string) {
        await test.step(`Validating visibility of element: ${elementName}`, async () => {
            try {
    ...
    async uploadMultipleContent(fileName1: string, fileName2: string, locator: any) {
        await test.step(`Uploading multiple files: ${fileName1}, ${fileName2}`, async () => {
    ...
    async samplefile(locator: string, Path: string) {
        await test.step(`Uploading sample file from path: ${Path}`, async () => {
    ...
    async uploadFile(locator: string, Path: string) {
        await test.step(`Uploading file from path: ${Path}`, async () => {
            const filePath = path.resolve(__dirname, Path);
            const inputElementHandle = this.page.locator(locator);
            if (inputElementHandle) {
    ...
    * 
    * @param {'minWait' | 'mediumWait' | 'maxWait'} waitType - The type of wait duration.
    */
    ...
    async spinnerDisappear(): Promise<void> {
        await test.step(`Waiting for spinner to disappear`, async () => {
            await this.wait('minWait');
    ...
    async handleAxisCoordinateClick(x_axis: number, y_axis: number) {
        await test.step(`The X-axis at ${x_axis} and Y-axis at ${y_axis} were clicked successfully.`, async () => {
            await this.wait('minWait');
    ...
    async childTab(locator: string): Promise<Page> {
        return await test.step(`Opening child tab via: ${locator}`, async () => {
    ...
    async clickwithnewInstance(selector: string) {
        return await test.step(`Clicking element in new tab: ${selector}`, async () => {
    ...
    async fillwithNewInstance(selector: string, data: string) {
        await test.step(`Filling element in new tab: ${selector} with data: ${data}`, async () => {
